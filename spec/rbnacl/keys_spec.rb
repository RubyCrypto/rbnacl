...
describe Crypto::SecretKey do
  ...
  context "new" do
    it "accepts a valid key" do
      expect { Crypto::SecretKey.new(bobsk) }.not_to raise_error(ArgumentError)
    end
    it "rejects a nil key" do
      expect { Crypto::SecretKey.new(nil) }.to raise_error(ArgumentError)
    end
    it "rejects a short key" do
      expect { Crypto::SecretKey.new("short") }.to raise_error(ArgumentError)
    end
  end
  ...
  context "valid?" do
    it "doesn't pass nil" do
      Crypto::SecretKey.valid?(nil).should be false
    end
    it "doesn't pass a short string" do
      Crypto::SecretKey.valid?("hello").should be false
    end
    it "does pass a valid key" do
      Crypto::SecretKey.valid?(bobsk).should be true
    end
  end
